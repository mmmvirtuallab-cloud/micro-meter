...
const MicrometerLab = () => {
    ...
    useEffect(() => {
        ...
        //create gui - Modified to use React ref
        const pane = new window.Tweakpane.Pane({ container: guiRef.current, title: "Micrometer Settings" });
        ...
        tab.pages[0].addInput(window, "msdValue", { label: "MSD", min: 0.05, max: 5, step: 0.5 }).on('change', update);
        tab.pages[0].addInput(window, "mainScaleDivisions", { label: "MSD Count", min: 10, max: 50, step: 10 }).on('change', update);
        var csController = tab.pages[0].addInput(window, "circularScaleDivisions", { label: "CSD Count", min: 25, max: 100, step: 25 });
        ...
        tab.pages[1].addInput(window, "objectShape", { label: "Shape", options: { rectangle: 'Rectangle', circle: 'Circle' } }).on('change', update);
        tab.pages[1].addInput(window, "objectRadius", { label: "Radius", min: 5, max: 50, step: 1 }).on('change', update);
        tab.pages[1].addInput(window, "objectHeight", { label: "Height", min: 20, max: 200, step: 10 }).on('change', update);
        ...
        problemFolder.addInput(window, "randomCircularScaleDivision", { label: "CSD Count" });
        problemFolder.addInput(window, "randomObjectWidthPixel", { label: "Object's Size" });
        problemFolder.addInput(window, "randomObjectShape", { label: "Shape" });
        ...
        paint();
        var intiScale = scale;
        tab.pages[0].addInput(window, "scale", { label: "Scale", min: 0.5, max: 3, step: 0.1 }).on('change', update);
